...
def yaml_search(key,input_object):
    """
    Search a key from yaml object, get nothing back if key is not found
    key : "keyword" to be searched, case sensitive
    input_object : yaml object to be parsed, test_data.py in this case
    ...
    def inner_function(key,input_object):
        ...
        else: # Iterate a list because some APIs return yaml object in a list
            for val in input_object:
                if not isinstance(val, (str,int)):
                    inner_function(key,val)
    inner_function(key,input_object)
    return ret_val
print(yaml_search("issueSummary",data))
